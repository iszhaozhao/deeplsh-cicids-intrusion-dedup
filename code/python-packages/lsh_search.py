...
#  根据指定的哈希表和查询索引，快速检索出所有可能的近重复样本索引（未去重）
...
# 将近重复样本的索引集合（set_indexes）转换为 similarities.py 中相似度矩阵的索引格式
...
# 计算广义 MRR（Mean Reciprocal Rank）：衡量检索结果中，近邻样本在排序中的平均位置质量
...
# 计算概率哈希最小元素：基于检索结果的最小相似度值，计算其在 LSH 哈希表中的命中概率
def prob_hashing_smallest_elt(approximate_nns_sim, K, L):
    return 1 - (1 - (approximate_nns_sim.values[-1])**K)**L

# 计算平均概率哈希最小元素：遍历所有样本，对每个样本使用指定的 LSH 参数组合，计算其检索结果的最小相似度概率，并取平均
...
# 计算召回率：遍历所有样本，对每个样本使用指定的 LSH 参数组合，计算其检索结果中 Top-K 近邻的召回率，并取平均
...
# 计算单个参数组合的召回率：遍历所有样本，对每个样本使用指定的 LSH 参数组合，计算其检索结果中 Top-K 近邻的召回率，并取平均
...
# 计算平均广义 MRR：遍历所有样本，对每个样本使用指定的 LSH 参数组合，计算其检索结果的广义 MRR，并取平均
...
# 计算所有参数组合的精确度：遍历所有样本，对每个样本使用指定的 LSH 参数组合，计算其检索结果的精确度，并取平均
...
# 计算所有参数组合的召回率：遍历所有样本，对每个样本使用指定的 LSH 参数组合，计算其检索结果的召回率，并取平均
...
# 计算所有参数组合的 F1 分数：基于精确度和召回率矩阵，计算每个参数组合的 F1 分数
...
# 计算最小哈希：对每个样本进行多次随机置换，提取非零元素的最小索引，并将其转换为二进制表示，生成哈希编码
...
# 计算 SimHash：基于帧的哈希编码和权重，计算每个样本的 SimHash 编码
...
# 创建 SimHash 哈希表：对每个组合，提取对应的哈希编码片段，生成哈希表
...
# 查找 SimHash 近似重复：对每个组合，提取对应的哈希编码片段，查找哈希表中相同的编码，返回索引集合
...
# 计算 SimHash 重叠率：遍历所有置换次数，对每个样本使用指定的 SimHash 参数组合，计算其检索结果的重叠率，并取平均
...
# 计算 SimHash 单参数重叠率：对指定置换次数，使用 SimHash 参数组合，计算每个样本的检索结果重叠率，并取平均
...
# 计算 SimHash 平均广义 MRR：遍历所有置换次数，对每个样本使用指定的 SimHash 参数组合，计算其检索结果的广义 MRR，并取平均
...
# 计算 SimHash 平均概率：遍历所有置换次数，对每个样本使用指定的 SimHash 参数组合，计算其检索结果的概率，并取平均
...
# 计算 SimHash 精度：遍历所有置换次数，对每个样本使用指定的 SimHash 参数组合，计算其检索结果的精度，并取平均
...
# 计算 SimHash 召回率：遍历所有置换次数，对每个样本使用指定的 SimHash 参数组合，计算其检索结果的召回率，并取平均
